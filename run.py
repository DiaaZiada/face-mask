...
def main():
    ...
    mask_detector = MaskDetector("./saved_model/face_mask/classifer_model.pth")
    ...
    if args.mode == 'image':
        ...
        for file_name in file_name_list:
            ...
            for bbox in bboxes:
                ...
                label, color = ("mask", (0,255,0)) if mask_detector(face) == 0 else ("no mask",(0, 0, 255))
                cv2.rectangle(im, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
                cv2.putText(im,label, (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color,2)
        ...
        for file_name in file_name_list:
            out_file = os.path.join(data_folder, file_name.replace('.mp4', '_result.avi'))
            ...
            while cv2.waitKey(1) < 0:
                ...
                for bbox in bboxes:
                    ...
                    start = time.time()
                    label, color = ("mask", (0,255,0)) if mask_detector(face) == 0 else ("no mask",(0, 0, 255))
                    total_time_mask_model += time.time() - start
                    cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
                    cv2.putText(frame,label, (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color,2)

                # computing_platform = 'Computing platform: CPU i7'
                ...
                # if total_time_mask_model:
                #     infer_speed_mask = 'Inference speed for mask model: %.2f FPS' % (1 / (total_time_mask_model/len(bboxes)))
                #     cv2.putText(frame, infer_speed_mask, (5, 195), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
